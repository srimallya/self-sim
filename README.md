...
The **NLRI** (`nlri/`) package is the main runtime and training system. It keeps the simulator itself as the generative world: `nlri/envs/maze_reservoir_env.py` exposes the same maze, grid scale, food-dot replenishment, moving agents, heading arrows, and realtime stepping through a gym-like API:
...
For immediate usability, the NLRI runtime supports a hybrid cold-start policy. The neural policy always produces logits, but when it is still effectively untrained it can fall back to the energy-gradient taxis behind `use_legacy_fallback=True`. That keeps the pygame loop visibly alive from the first run instead of making visual behavior depend on completed training.

Run the NLRI realtime pygame simulation with:
